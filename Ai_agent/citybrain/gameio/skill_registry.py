...
class SkillRegistry:
    ...
    def register_skill_from_code(self, skill_code: str, overwrite = False) -> Tuple[bool, str]:
        ...
        print("SKILL_EMBEDDING_KEY embedding 1:",len(self.get_embedding(skill_name, inspect.getdoc(skill))))
    ...
    def retrieve_skills(self, query_task: str, skill_num: int, screen_type: str) -> List[str]:
        ...
        print("target_skills0:",len(target_skills))
        task_emb = np.array(self.embedding_provider.embed_query(query_task))
        #print("Skill embedding dimension:",len(self.skill_index[0][SKILL_EMBEDDING_KEY]))
        #print("task embedding dimension:",len(task_emb))
        ...
        self.recent_skills = []
        print("target_skills1:",len(target_skills))
        ...
        print("this is skill_index4:", len(self.skill_index))
        print("target_skills2:",len(target_skills))
    ...
    def load_skill_library(self, file_name) -> None:
        ...
        self.skill_index = []
        self.skill_registry = {}
        print("this is skill_index7.:", len(self.skill_index))
        for skill_name in skill_local.keys():
            ...
            if skill_name in SKILL_REGISTRY:
                ...
                if skill_code_base64 == skill_local[skill_name][SKILL_CODE_HASH_KEY]: # the skill_code is not modified
                    print("SKILL_EMBEDDING_KEY embedding 2.:",len(skill_local[skill_name][SKILL_EMBEDDING_KEY]))

                    tken=np.frombuffer(self.get_embedding(skill_name, 'Moves the camera forward in the game at a specified distance.'))
                    tken=base64.b64encode(tken.tobytes()).decode('utf-8')
                    # print("this is 2.:",tken)
                    self.skill_index.append({SKILL_NAME_KEY:skill_name,
                    ...
                    print("SKILL_EMBEDDING_KEY embedding2:", len(self.get_embedding(skill_name,  inspect.getdoc(SKILL_REGISTRY[skill_name]))))
                    self.skill_index.append({SKILL_NAME_KEY:skill_name,
                ...
                if skill_code_base64 == skill_local[skill_name][SKILL_CODE_HASH_KEY]: # the skill_code is not modified
                    print("SKILL_EMBEDDING_KEY embedding 3.:",len(np.frombuffer(base64.b64decode(skill_local[skill_name][SKILL_EMBEDDING_KEY]), dtype=np.float64)))
                    self.skill_index.append({SKILL_NAME_KEY:skill_name,
                                             SKILL_EMBEDDING_KEY:np.frombuffer(base64.b64decode(skill_local[skill_name][SKILL_EMBEDDING_KEY]), dtype=np.float64),
                                             SKILL_CODE_KEY:skill_local[skill_name][SKILL_CODE_KEY]})

                else: # skill_code has been modified, we should recompute embedding
                    print("SKILL_EMBEDDING_KEY embedding 3:",len(self.get_embedding(skill_name, inspect.getdoc(SKILL_REGISTRY[skill_name]))))
                    self.skill_index.append({SKILL_NAME_KEY:skill_name,
                                             SKILL_EMBEDDING_KEY:self.get_embedding(skill_name, inspect.getdoc(skill)),
                                             SKILL_CODE_KEY:skill_local[skill_name][SKILL_CODE_KEY]})
    ...
    def filter_skill_library(self) -> None:
        print("this is skill_index8.:", len(self.skill_index))
        if self.skill_scope == 'Basic':
            ...
            for skill in SKILL_INDEX:
                if skill[SKILL_NAME_KEY] in self.basic_skills:
                    ...
                    print("SKILL_EMBEDDING_KEY embedding 4:",len(skill[SKILL_EMBEDDING_KEY]))
        ...
        if self.skill_scope == None:
            ...
            for skill in SKILL_INDEX:
                if skill[SKILL_NAME_KEY] in self.necessary_skills:
                    print("SKILL_EMBEDDING_KEY embedding 5:",len(skill[SKILL_EMBEDDING_KEY]))
                    self.skill_registry[skill[SKILL_NAME_KEY]] = SKILL_REGISTRY[skill[SKILL_NAME_KEY]]
                    self.skill_index.append(skill)
