...
from  citybrain.VLM import LLMProvider, EmbeddingProvider
from  citybrain.config import Config
from  citybrain.log import Logger
from  citybrain.utils.json_utils import load_json
from  citybrain.utils.encoding_utils import encode_base64, decode_base64
...
PROVIDER_SETTING_IS_Gemini = "is_gemini"
...
class OpenAIProvider(LLMProvider, EmbeddingProvider):
    ...
    def _parse_config(self, provider_cfg) -> dict:
        ...
        if conf_dict[PROVIDER_SETTING_IS_AZURE]:
        ...
        elif conf_dict[PROVIDER_SETTING_IS_Gemini]:
            key = os.getenv(key_var_name)
            self.client=genai.configure(api_key=key , transport="rest")
            ...
            # key = os.getenv(key_var_name)
            # print(key)
            # self.client = OpenAI(api_key=key, base_url=conf_dict[PROVIDER_SETTING_BASE_VAR])
    ...
    def embed_with_retry(self, **kwargs: Any) -> Any:
        ...
        def _embed_with_retry(**kwargs: Any) -> Any:
            if "gemini" in self.model_type:
                response = self.client.embed_content(**kwargs)
            else:
    ...
    def get_embedding_dim(self) -> int:
        ...
        if self.embedding_model == "text-embedding-ada-002":
            embedding_dim = 1536
        elif self.embedding_model == "text-embedding-004":
            embedding_dim = 768
