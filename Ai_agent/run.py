'''这段代码是一个主函数，它通过调用各种模块和服务来执行游戏策略。主要流程如下：

1. 初始化各种服务和模块，包括语言模型服务、游戏管理、视频处理等。
2. 进入游戏界面，并开始录制游戏视频。
3. 在游戏中循环执行以下步骤：
   - 收集游戏状态信息。
   - 根据收集到的信息进行决策制定。
   - 执行决策，执行游戏操作。
   - 判断执行结果，是否达到预期的成功条件。
4. 当成功条件满足或者手动中断时，结束游戏循环。

这段代码主要用于自动化游戏策略的执行，通过不断地收集游戏状态信息、进行决策制定和执行，以及判断执行结果，从而实现自动化的游戏操作。
'''
import os
import time
import copy
...
from citybrain.gameio.game_manager import GameManager
from citybrain.log import Logger
from citybrain.agent import Agent
from citybrain.planner.planner import Planner
from citybrain.memory import LocalMemory
from citybrain.VLM.openai import OpenAIProvider
from citybrain.VLM import GdProvider
from citybrain.gameio.io_env import IOEnvironment
from citybrain.gameio.lifecycle.ui_control import switch_to_game, IconReplacer
from citybrain.gameio.video.VideoRecorder import VideoRecorder
from citybrain.gameio.video.VideoFrameExtractor import VideoFrameExtractor
from citybrain.gameio.atomic_skills.trade_utils import __all__ as trade_skills
from citybrain.gameio.atomic_skills.buy import __all__ as buy_skills
from citybrain.gameio.atomic_skills.map import __all__ as map_skills
from citybrain.gameio.atomic_skills.move import __all__ as move_skills
from citybrain.gameio.atomic_skills.combat import __all__ as combat_skills
